...
class LibriSpeechDataset(Sequence):
    ...
    def __init__(self, subsets, seconds, label='speaker', stochastic=True, pad=False, cache=True):
        ...
        if cache:
            ...
            for s in subsets:
                subset_index_path = PATH + '/../LibriSpeech/{}.index.csv'.format(s)
        ...
        for s in subsets:
            self.df[self.df['subset'] == s].to_csv(PATH + '/../LibriSpeech/{}.index.csv'.format(s), index=False)
    ...
    def build_verification_batch(self, batchsize):
        ...
        for i in range(batchsize/2):
            print(self.datasetid_to_filepath[alike_pairs[i][0]])
            print(self.datasetid_to_filepath[alike_pairs[i][1]])
            print('\n')
        ...
        for i in range(batchsize/2):
            print(self.datasetid_to_filepath[differing_pairs[i][0]])
            print(self.datasetid_to_filepath[differing_pairs[i][1]])
            print('\n')
